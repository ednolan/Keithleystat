...
function parseIEEE754(str) {
	ieee_sign = str[0]
	ieee_num = str.substring(1,9)
	ieee_powsign = str[10]
	ieee_pow = str.substring(11,13)
	if(ieee_num == 0) ieee_parsed = 0
	else if (ieee_num == "9.910000" && ieee_pow == "37") ieee_parsed = "NaN"
	else {
		if(ieee_sign == "+") ieee_mult = 1
		else ieee_mult = -1
		if(ieee_powsign == "+") ieee_powmult = 1
		else ieee_powmult = -1
		ieee_parsed = ieee_mult * parseFloat(ieee_num) * Math.pow(10,ieee_powmult*parseInt(ieee_pow, 10))
	}
	return ieee_parsed
}
...
//sent
var queuer = []
function toArd(command)
{
	queuer.push(command + "\r")
